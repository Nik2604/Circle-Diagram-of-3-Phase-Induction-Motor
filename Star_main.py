...
class Star:
    def __init__(self, rated_voltage, poles, frequency, power, voltage_np, current_np, power_np, voltage_br, current_br, power_br, stator_rotor_ratio, scale):
        ...
        self.scale = scale
        self.current_oc = current_np/self.scale

        self.theta_np = (math.acos((self.power_np) /
                                   (((math.sqrt(3))*self.voltage_np*self.current_np))))
        self.theta_br = (math.acos((self.power_br) /
                                   (((math.sqrt(3))*self.voltage_br*self.current_br))))
        self.current_sc = (
            (self.rated_voltage*self.current_br)/self.voltage_br)/self.scale
        ...
        self.powerscale_pp = (self.rated_voltage/math.sqrt(3))*self.scale
    ...
    def draw_star(self):
        basex = np.array([0, self.dscx + 20/self.scale, 0, 0])
        basey = np.array([0, 0, 0, self.dscy + 20/self.scale])
        ie_x = np.array([0, self.qnpx])
        ie_y = np.array([0, self.qnpy])
        plt.arrow(0, 0, 0, self.dscy + 20 /
                  self.scale, head_width=1/self.scale, head_length=2/self.scale, fc='#0099ff', ec='#0099ff', color='#0099ff')
        ...
        plt.text(0, self.dscy + 20/self.scale, 'V', **self.text_kwargs)
        plt.show()
